...
<div class="container mt-0">
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="mb-4 text-center" style="font-weight: bold;">Testimoni</h2>
            <!-- Table -->
            <table class="table table-hover table-striped table-bordered text-center" style="box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); border-radius: 4px; overflow: hidden;">
                <thead>
                    <tr>
                        ...
                        <th scope="col" style="font-size: 1em;">Desctiption</th>
                        <th scope="col" style="font-size: 1em;">Rating</th>
                ...
                <tbody>
                    <?php $no = 0; ?>
                    <?php foreach ($testimonicontroller as $item): ?>
                    <tr>
                        <td><?= $no += 1; ?></td>
                        <td><?= implode(' ', array_slice(explode(' ', $item['name']), 0, 80)) . (str_word_count($item['name']) > 10 ? '...' : '') ?></td>
                        <td><?= implode(' ', array_slice(explode(' ', $item['desc']), 0, 80)) . (str_word_count($item['desc']) > 10 ? '...' : '') ?></td>
                        <td>
                            <div class="rating">
                                <?php
                                $rating = (int) $item['rating']; // Pastikan rating berupa integer
                                for ($i = 0; $i < 5; $i++) {
                                    if ($i < $rating) {
                        ...
                        <td class="text-center align-middle" style="height: 100px;">
                            <div class="d-flex justify-content-center align-items-center" style="gap: 4px;">
                                ...
                                <form action="/testimonicontroller/<?= $item['id'] ?>" method="POST" 
